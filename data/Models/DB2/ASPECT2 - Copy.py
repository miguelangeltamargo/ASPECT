...
def extended_compute_metrics(eval_pred):
    ...
    # predictions are raw logits. Convert to probabilities for AUC/PR-AUC
    ...
    # Binary classification metrics for Alternative Splice label as posotive class
    # By default positive label is set to 1 - These 3 metrics are all focused on Alternative Spliced Exon
    precision, recall, f1, _ = precision_recall_fscore_support(labels, pred_class, average="binary", pos_label=1, zero_division=0)
    macro_precision, macro_recall, macro_f1, _ = precision_recall_fscore_support(labels, pred_class, average="macro", zero_division=0)    # Macro (unweighted), Weighted, and Per-Class Metrics
    weighted_precision, weighted_recall, weighted_f1, _ = precision_recall_fscore_support(labels, pred_class, average="weighted", zero_division=0)
    per_class_metrics = precision_recall_fscore_support(labels, pred_class, average=None, zero_division=0)  # Per-class

    # For binary AUC
    # Check if problem is binary
    ...
    class_report = classification_report(labels, pred_class, target_names=["Constitutive", "Alternative 5"], output_dict=True)
    ...
    return {
        ...
        "per_class_precision": per_class_metrics[0].tolist(),  # Precision per class
        "per_class_recall": per_class_metrics[1].tolist(),     # Recall per class
        "per_class_f1": per_class_metrics[2].tolist(),         # F1-Score per class
        
        "auc": auc_value,
        "pr_auc": pr_auc_value,
        
        "class_report": class_report  # Full classification report for debugging
    }
    ...
    try:
        if model_args.use_optuna:
            ...
            def objective(trial):
                ...
                epochs = trial.suggest_int("num_train_epochs", 5, 10)
                ...
                alpha1 = trial.suggest_float("alpha_class_0", 0.1, 0.5)  # constitutive class
                alpha2 = trial.suggest_float("alpha_class_1", 0.5, 1.0)  # minority class
