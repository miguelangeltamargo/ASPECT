...
# Output File
dataset_filename = f"../datasets/{OFFSET}_Split/{OFFSET}_Split_C_5.csv"
...
def combine_data_to_bed():
    ...
    try:
        ...
        data2 = pd.read_csv(input_2, sep="\t", low_memory=False)    # minority
    ...
    constitutive_filtered = const_filtered.sample(40000)
    ...
    # -----------------------------------------------------------------------------------
        ...
        minority_filtered = pd.concat([cassette_filtered_low, cassette_filtered_moderate], ignore_index=True)
        ...
        print(f"✅ Cassette Exons passed: {minority_filtered.shape[0]}/{data2.shape[0]}")
    elif ALT3:
    # -----------------------------------------------------------------------------------
    # 📌 **Apply Filtration to Alt3 Exons**
    # -----------------------------------------------------------------------------------
        print("📋 Filtering Alternatve 3' Exons...")

        # Define inclusion level categories
        incl_lvl = (data2['3usageLevel'] > 0.15) & (data2['3usageLevel'] <= 0.90)
        
        # Apply filters for both low and moderate inclusion levels
        alt3_filtered = data2[
            incl_lvl &
            (data2['count'] >= 4) &  # Assuming 'alt3' means at least 20 skip
            (data2['alt3'] > 0) &
            # (data2['alt5'] + data2['alt3+5'] <= 5) &
            (data2['exon_length'] >= 25)
        ].copy()

        # Combine low and moderate inclusion cassette exons
        minority_filtered = pd.concat([alt3_filtered], ignore_index=True)
        

        print(f"✅ Alt 3' Exons passed: {minority_filtered.shape[0]}/{data2.shape[0]}")
        
    elif ALT5:
    # -----------------------------------------------------------------------------------
    # 📌 **Apply Filtration to Alt5 Exons**
    # -----------------------------------------------------------------------------------
        print("📋 Filtering Alternatve 5' Exons...")

        # Define inclusion level categories
        # incl_lvl = (data2['5usageLevel'] > 0.15) & (data2['5usageLevel'] <= 0.90)
        incl_lvl = (data2['5usageLevel'] <= 0.90)
        
        # Apply filters for both low and moderate inclusion levels
        alt3_filtered = data2[
            incl_lvl &
            (data2['count'] >= 4) &  # Assuming 'alt5' means at least 20 skip
            (data2['alt5'] > 0) &
            # (data2['alt5'] + data2['alt3+5'] <= 5) &
            (data2['exon_length'] >= 25)
        ].copy()
        ...
        print(f"✅ Alt 5' Exons passed: {minority_filtered.shape[0]}/{data2.shape[0]}")
    ...
    # Process minority exons
    bed_data2 = minority_filtered[["chromo", "start", "end", "strand"]].copy()
    ...
    bed_data2["label"] = "1"  # Label for minority exons
