...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # weight_init: str = field(default="xavier")
...
def train():
    ...
    if tokenizer is None:
        ...
        logger.info("Created and Loaded Tokenizer successfully.")
    else:
    ...
    train_data_path = os.path.join(
        data_args.data_path, 
        "train_oversampled.csv" if model_args.use_oversampled else "train.csv"
    )
    ...
    def build_and_train_model(learning_rate, weight_decay, num_train_epochs, batch_size, 
                              alpha=None, gamma=2.0, scheduler="cosine_with_restarts", 
                              warmup_steps=500, weight_init="xavier", beta1=0.9, beta2=0.999):
        ...
        # def override_dropout(model, dropout_value):
        #     for module in model.modules():
        #         if isinstance(module, torch.nn.Dropout):
        #             module.p = dropout_value

        # override_dropout(model, dropout)
        ...
        local_training_args.lr_scheduler_type = scheduler
        local_training_args.warmup_steps = warmup_steps

        # optimizer_kwargs = {
        #     "betas": (beta1, beta2)
        # }

        local_training_args.max_grad_norm = 1.0
        local_training_args.load_best_model_at_end = True
        local_training_args.metric_for_best_model = "eval_macro_f1"
    ...
    def evaluate_best_model(run_name: str, model_args, training_args, test_dataset, tokenizer):
        """Evaluate best model from Optuna trials on test set."""
        
        # Load best model checkpoint
        best_model_dir = os.path.join(f"./results/{run_name}/", f"{run_name}_Best_Model")
        ...
        # Apply LoRA if used in training
        if model_args.use_lora:
            from peft import PeftModel
            model = PeftModel.from_pretrained(model, best_model_dir)

        # Load best hyperparameters
        with open(os.path.join(best_model_dir, f"{run_name}_best_params.json"), "r") as f:
            best_params = json.load(f)

        # Configure loss function
        if model_args.use_class_weights:
            alpha = torch.tensor([best_params["alpha_class_0"], best_params["alpha_class_1"]])
        ...
        # Save results
        results_dir = os.path.dirname(best_model_dir)
        with open(os.path.join(results_dir, "final_test_results.json"), "w") as f:
            json.dump(test_results, f)

        # Generate additional metrics/plots
        predictions, labels, _ = eval_trainer.predict(test_dataset)
        plot_confusion_matrix(eval_trainer, test_dataset, results_dir, run_name)
    ...
    try:
        if model_args.use_optuna:
            ...
            def objective(trial):
                ...
                # Hyperparameter suggestions (example ranges)
                lr = trial.suggest_float("learning_rate", 5e-5, 9e-5, log=True)
                wd = trial.suggest_float("weight_decay", 5e-5, 1.5e-4, log=True)
                epochs = trial.suggest_int("num_train_epochs", 8, 12)
                bs = trial.suggest_categorical("batch_size", [16, 32])
                
                gamma = trial.suggest_float("gamma", 4, 5.5)
                alpha1 = trial.suggest_float("alpha_class_0", 0.1, 0.4)
                alpha2 = trial.suggest_float("alpha_class_1", 0.45, 2)
                ...
                #   Training and Validation Point
                trainer = build_and_train_model(
                    lr, wd, epochs, bs, alpha=alpha, gamma=gamma)
                
                # Get metrics from last validation run (already computed)
                ...
                try:
                    ...
                    best_model_dir = os.path.join(output_dir, f"{run_name}_Best_Model")
                    best_params_file = os.path.join(best_model_dir, f"{run_name}_best_params.json")
                    
                    eval_macro_f1 = metrics.get("eval_macro_f1", 0)
                    eval_auc = metrics.get("eval_auc", 0)
                    eval_class_1_f1 = metrics.get("class_1_f1", 0)
                    
                    if eval_macro_f1 is None or eval_auc is None or eval_class_1_f1 is None:
                        raise ValueError(f"Metrics are missing required keys: {metrics}")

                    # combined_score = 0.3 * eval_macro_f1 + 0.4 * eval_auc + eval_class_1_f1 * 0.3
                    # combined_score = 0.1 * eval_macro_f1 + 0.5 * eval_auc + eval_class_1_f1 * 0.4 # for next run
                    combined_score = 0.4 * eval_auc + eval_class_1_f1 * 0.6 # for next run
                    current_score = combined_score
                    
                    if trial.number == 0 or (trial.study.best_trial is not None and current_score > trial.study.best_value):
                        logger.info(f"New best trial found: {trial.number} with combined_score : {current_score}")
                        trainer.save_state()
                        safe_save_model_for_hf_trainer(trainer=trainer, output_dir=best_model_dir)
                        with open(best_params_file, "w") as f:
                            json.dump(study.best_params, f)
                        logger.info(f"Best params saved at {best_params_file}")
                except Exception as e:
                    logger.error(f"An error occurred Saving best trial model to disk: {e}")
                ...
                logger.info(f"Combined Score: {current_score}\n")
            ...
            for trial_data in sorted(trial_results, key=lambda x: x['combined_score'], reverse=True):
                logger.info(f"\nTrial {trial_data['trial_number']}:")
                logger.info(f"Combined Score: {trial_data['combined_score']:.4f}")
                logger.info("Key Metrics:")
                logger.info(f"  Macro F1: {trial_data['metrics']['eval_macro_f1']:.4f}")
                logger.info(f"  Micro F1: {trial_data['metrics']['eval_micro_f1']:.4f}")
                logger.info(f"  AUC: {trial_data['metrics']['eval_auc']:.4f}")
                logger.info(f"  PR-AUC: {trial_data['metrics']['eval_pr_auc']:.4f}")
                logger.info(f"  Class 1 F1: {trial_data['metrics']['eval_class_1_f1']:.4f}")
                logger.info(f"  Binary F1: {trial_data['metrics']['eval_binary_f1']:.4f}")
            ...
            try:
                ...
                if model_args.use_optuna:
                    ...
                    # Evaluate best model
                    final_results = evaluate_best_model(
                        run_name=run_name,
