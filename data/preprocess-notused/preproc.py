...
for i in range(len(data) - 1):  # Iterate until the second-to-last row
    ...
    if count >= 20 and (end - start) >= 25:
        ...
        if pSkip == 1 and (next_start - end) >= 80:
            ...
            new_values = [item.decode('cp1252') for item in current_row]
            new_values[5] = str(start - prev_end)#legnth of left intro
            new_values[6] = str(end - start)#length of exon
            new_values[7] = str(next_start - end)#length of right intro
            new_values[8] = str(start - 70)#left junct1
            new_values[9] = str(start + 70)#left junct2
            new_values[10] = str(end - 70)#right junct1
            new_values[11] = str(end + 70)#right junct2
            filtered_data.append(new_values)
...
input_file = 'filt_ard.tsv'

# Specify the path to the output BED file
output_file = 'filt_junc.bed'
...
print(f"Converted BED file saved as: {bed6.filename}")